...
/**
 * @author Алексей Соколов on 10.08.2022
 */
class GroupProfileToolbarMenuProvider @AssistedInject constructor(
    @AppContext appContext: Context,
    @Assisted params: ToolbarGroupProfileCustomParams,
) : ToolbarMenuProvider {
    private val customButtonInfo: CustomButtonInfo = CustomButtonInfo(
        mainDrawable = R.drawable.ic_custom_action_24dp,
        text = appContext.getString(R.string.group_profile_custom_action)
    ) {
        Toast.makeText(
            appContext,
            appContext.getString(R.string.group_profile_custom_toast, params.toString()),
            Toast.LENGTH_LONG
        ).show()
    }
